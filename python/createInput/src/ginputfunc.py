...
def create_cfe_input(
    ...
    for catID in catids:
        ...
        f.write("%s" % ("max_gw_storage=" + str(dfa.loc[catID]["gw_Zmax"] / 1000.0) + "[m]\n"))
...
def create_sft_smp_input(
    ...
    for catID in catids:
        ...
        sft_lst = [
            ...
            "soil_params.quartz="
            + str(dfa.loc[catID][[x for x in dfa.columns.to_list() if "quartz" in x]].mean())
            + "[]",
...
def create_snow17_input(catids: List[str], attr_file: Union[str, Path], snow17_input_dir: str) -> None:
...
def create_ueb_input(
    ...
    # make sure either bmi_dir or attr_file is provided
    if (not bmi_dir or bmi_dir == "") and not Path(attr_file).exists():
        raise Exception(
            "Either a valid path to bmi_dir or a domain attribute file must be provided for UEB to create input files."
        )

    # sitevars file
    for catID in catids:
        site_file = os.path.join(ueb_input_dir, "ueb_sitevars-" + catID + ".dat")
        if bmi_dir and bmi_dir.strip():  # if bmi_dir is provided, use BMI files in the folder
        ...
        else:  # otherwise, create the sitevars file based on a template and the attribute file
...
def create_sac_input(catids: List[str], attr_file: Union[str, Path], sac_input_dir: str) -> None:
    """Create BMI configuration file for Snow17
...
def create_lstm_config(input_config_path: str, output_dir: str, params_to_remove=None, params_to_update=None) -> None:
    """
...
def create_pet_input(catids: List[str], attr_file: Union[str, Path], pet_input_dir: str) -> None:
...
def create_realization_file(
    ...
    t = {
        "time": {
            "start_time": time_period["run_time_period"]["calib"][0],
            "end_time": time_period["run_time_period"]["calib"][1],
...
def create_partition_file(
    partition_generator: str, hydrofab_file: str, nprocs: int, work_dir: str, basin: str
) -> Union[str, Path]:
