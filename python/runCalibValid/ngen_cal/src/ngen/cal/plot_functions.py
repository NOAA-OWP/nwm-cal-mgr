...
def plot_streamflow_precipitation(
    ...
    cols = ["black", "blue", "orange", "tab:green", "tab:cyan", "purple"]  # ['k','C1','C0','C3']
...
def scatterplot_streamflow(
    ...
    """
    logger.debug("---Plotting Scatterplot of Streamflow between Observation and Other Runs---")
...
def scatterplot_objfun(
    ...
    if best_iteration is not None:
        (bestpt,) = ax.plot(
            best_iteration, df.loc[df["iteration"] == best_iteration, [objective_fun_column]], "r*", markersize=10
        )
...
def scatterplot_var(
    ...
    for ax, varname in zip(axs, allcols):
        ...
        if best_iteration is not None:
            (bestpt,) = ax.plot(
                best_iteration, df.loc[df["iteration"] == best_iteration, [varname]], "ro", markersize=5
            )
...
def plot_fdc_valid(
    ...
    fig, ax = plt.subplots(figsize=(12.2, 5.8), nrows=1, ncols=3, sharey=True, tight_layout=True)
