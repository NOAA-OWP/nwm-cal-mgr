...
def plot_streamflow(
    ...
    max0 = (
        math.ceil(pd.melt(df, id_vars=["Time"], value_vars=colname[1:])["value"].max())
        * 1.02
    )
    ...
    if (
        start_calib is not None
        and end_calib is not None
        and start_valid is not None
        and end_valid is not None
    ):
        ax.axvline(
            x=pd.DatetimeIndex([start_calib]), color="0.8", linestyle="--", linewidth=3
        )
        ax.axvline(
            x=pd.DatetimeIndex([end_calib]), color="0.8", linestyle="--", linewidth=3
        )
        if end_calib != start_valid:
            ax.axvline(
                x=pd.DatetimeIndex([start_valid]),
                color="0.8",
                linestyle="--",
                linewidth=3,
            )
        ax.axvline(
            x=pd.DatetimeIndex([end_valid]), color="0.8", linestyle="--", linewidth=3
        )
...
def plot_streamflow_precipitation(
    ...
    try:
        ts = (
            ...
            else pd.DatetimeIndex(dfp[colname[0]])
        )
    ...
    max0 = (
        math.ceil(
            pd.melt(df, id_vars=colname[0], value_vars=colname[1:])["value"].max()
        )
        * 1.02
    )
    ...
    for i, x in enumerate(colname[1:]):
        lnlst.append(
            ax.plot(
                df.Dates, df[x], c=cols[colname[1:].index(x)], label=x, linewidth=0.8
            )
        )
    ...
    ax2 = ax.twinx()
    ln3 = ax2.plot(
        dfp.Dates, dfp["RAINRATE"], color=cols[-1], label="Precipitation", linewidth=0.8
    )
    # ax2.invert_yaxis()
...
def scatterplot_streamflow(
    ...
    for x in colname[2:]:
        ax.scatter(
            ...
            label=x,
            facecolors="none",
            edgecolors=cols[colname[2:].index(x)],
            marker="o",
        )
    ...
    ax.set_xlabel(
        r"$\mathsf{Observed}\ \mathsf{Streamflow}\ (\mathsf{m^3}/\mathsf{s})$",
        fontsize=18,
    )
    ax.set_ylabel(
        r"$\mathsf{Simulated}\ \mathsf{Streamflow}\ (\mathsf{m^3}/\mathsf{s})$",
        fontsize=18,
    )
    if title is not None:
...
def plot_output(
    ...
    if "Q_OUT" in output_name:
        ylabels = [
            r"$\mathsf{Streamflow}\ (\mathsf{m^3}/\mathsf{s})$",
            r"$\mathsf{Direct}\ \mathsf{Runoff}\ (\mathsf{m^3}/\mathsf{s})$",
            r"$\mathsf{GIUH}\ \mathsf{Runoff}\ (\mathsf{m^3}/\mathsf{s})$",
            r"$\mathsf{Lateral}\ \mathsf{Runoff}\ (\mathsf{m^3}/\mathsf{s})$",
            r"$\mathsf{Baseflow}\ (\mathsf{m^3}/\mathsf{s})$",
            r"$\mathsf{Precipitation}\ (\mathsf{mm}/\mathsf{h})$",
        ]
    if "Qout" in output_name:
        ylabels = [
            r"$\mathsf{Streamflow}\ (\mathsf{m^3}/\mathsf{s})$",
            r"$\mathsf{Total}\ \mathsf{Discharge}\ (\mathsf{m^3}/\mathsf{s})$",
            r"$\mathsf{Overland}\ \mathsf{Flow}\ (\mathsf{m^3}/\mathsf{s})$",
            r"$\mathsf{Baseflow}\ (\mathsf{m^3}/\mathsf{s})$",
            r"$\mathsf{Recharge}\ \mathsf{Flow}\ (\mathsf{m^3}/\mathsf{s})$",
            r"$\mathsf{Precipitation}\ (\mathsf{mm}/\mathsf{h})$",
        ]
    ...
    max1 = (
        math.ceil(
            pd.melt(df, id_vars=["Time"], value_vars=[*list1, "Observation"])[
                "value"
            ].max()
        )
        * 1.04
    ...
    for i in range(len(colname)):
        ...
        if i == 0:
            ax[i].plot(
                df["Dates"], df[["Observation"]], c=cols[2], label=lab[2], linewidth=1
            )
        ...
        if i == 5:
            ...
            ax[i].xaxis.set_label_coords(0.5, -0.5)
...
def scatterplot_objfun(
    ...
    if best_iteration is not None:
        (bestpt,) = ax.plot(
            best_iteration,
            df.loc[df["iteration"] == best_iteration, [objective_fun_column]],
            "r*",
            markersize=10,
        )
        ax.legend(
            handles=[bestpt],
            labels=["Best Iteration"],
            loc="upper right",
            frameon=False,
        )
...
def scatterplot_var(
    ...
    for ax, varname in zip(axs, allcols):
        ...
        if best_iteration is not None:
            (bestpt,) = ax.plot(
                best_iteration,
                df.loc[df["iteration"] == best_iteration, [varname]],
                "ro",
                markersize=5,
            )
        ...
        ax.set_title(varname) if len(varname) < 20 else ax.set_title(
            "_".join(varname.split("_")[:2])
        )
        ax.grid(True, color="0.7", linewidth=0.6)
    ...
    plt.subplots_adjust(
        left=0.06, right=0.95, bottom=0.08, top=0.86, hspace=0.25, wspace=0.38
    )
...
def scatterplot_objfun_metric(
    ...
    for ax, varname in zip(axs, allcols):
        ...
        if best_iteration is not None:
            (bestpt,) = ax.plot(
                df.loc[idx, [objcol]], df.loc[idx, [varname]], "ro", markersize=5
            )
            if varname == allcols[0]:
                ax.legend(
                    handles=[bestpt],
    ...
    plt.suptitle(title, size=20, weight="bold")
    plt.subplots_adjust(
        left=0.06, right=0.95, bottom=0.08, top=0.85, hspace=0.25, wspace=0.38
    )
...
def barplot_metric(
    ...
    for ax, varname in zip(axs, allcols):
        ...
        for i in range(len(runtp)):
            ax.bar(
                xwidth[i],
                df.loc[(runtp[i], labels), varname].values.tolist(),
                width,
                label=label0[i],
            )
    ...
    plt.suptitle(title, size=20, weight="bold")
    plt.subplots_adjust(
        left=0.06, right=0.94, bottom=0.12, top=0.85, hspace=0.25, wspace=0.35
    )
...
def plot_fdc_calib(
    ...
    fig, ax = plt.subplots(figsize=(10, 6), nrows=1, ncols=1, tight_layout=True)
    for i in range(len(colname)):
...
def plot_fdc_valid(
    ...
    fig, ax = plt.subplots(
        figsize=(12.2, 5.8), nrows=1, ncols=3, sharey=True, tight_layout=True
    )
...
def plot_cost_hist(
    ...
    for x in colname:
        ax.plot(
            np.arange(0, len(df)), df[x], c=cols[x], label=cost_name[x], linewidth=1
        )
