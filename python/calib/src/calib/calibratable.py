        ...
        if any([m in self._adf["model"].unique() for m in mlst]):
            for m in mlst:
                if ((self._adf["model"] == m) & (self._adf["param"] == snm)).any():
                    self._adf.loc[
                        (self._adf["model"] == m) & (self._adf["param"] == snm), "param"
                    ] = dnm

    def df_fill(self, iteration: int) -> None:
        """Fill all parameter DataFrame with unique parameter DataFrame"""
        filter_condition = (
            "LASAM" not in self._adf["model"].unique()
            or "CFE" in self._adf["model"].unique()
        )
        if iteration > 0:
            if filter_condition:
...
class Evaluatable(ABC):
    ...
    def write_hist_file(
        self, optimizer_result: "SwarmOptimizer", agent: "Agent", params_lst: list
    ) -> Path:
