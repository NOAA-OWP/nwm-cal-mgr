...
from calib.model import EvaluationOptions
from calib.ngen import Ngen
...
@pytest.fixture(scope="session")
def realization_config(tmp_path_factory) -> str:
    """
    Fixture to provide a staged testing input files
    """
    fn = tmp_path_factory.mktemp("data").joinpath("realization_config.json")
...
def ngen_config(realization_config, workdir) -> Ngen:
    ...
    model = Ngen.model_validate(ngen_config)
    return model
...
def catchment(nexus, fabric, workdir, mocker) -> Generator[CalibrationCatchment, None, None]:
    ...
    mocker.patch(__name__ + ".EvaluatableCatchment.output", new_callable=mocker.PropertyMock, return_value=output)
    # Disable output saving for testing purpose
    mocker.patch(__name__ + ".AdjustableCatchment.save_output", return_value=None)
